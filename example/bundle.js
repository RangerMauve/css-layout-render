...
},{"css-layout":3,"par":4}],3:[function(require,module,exports){
/**
...
var computeLayout = (function() {
    ...
    function isFlex(node) {
        return (
        getPositionType(node) === CSS_POSITION_RELATIVE && getFlex(node) > 0);
    }
    ...
    function layoutNode(node, parentMaxWidth) {
        ...
        var /*css_flex_direction_t*/
        mainAxis = getFlexDirection(node);
        var /*css_flex_direction_t*/
        crossAxis = mainAxis === CSS_FLEX_DIRECTION_ROW ? CSS_FLEX_DIRECTION_COLUMN : CSS_FLEX_DIRECTION_ROW;
        ...
        if (isMeasureDefined(node)) {
            ...
            var /*bool*/
            isColumnUndefined = !isDimDefined(node, CSS_FLEX_DIRECTION_COLUMN) && isUndefined(node.layout[dim[CSS_FLEX_DIRECTION_COLUMN]]);
        ...
        for (i = 0; i < node.children.length; ++i) {
            ...
            if (getAlignItem(node, child) === CSS_ALIGN_STRETCH && getPositionType(child) === CSS_POSITION_RELATIVE && !isUndefined(node.layout[dim[crossAxis]]) && !isDimDefined(child, crossAxis)) {
                ...
                node.layout[dim[crossAxis]] - getPaddingAndBorderAxis(node, crossAxis) - getMarginAxis(child, crossAxis),
                ...
                for (ii = 0; ii < 2; ii++) {
                    ...
                    if (!isUndefined(node.layout[dim[axis]]) && !isDimDefined(child, axis) && isPosDefined(child, leading[axis]) && isPosDefined(child, trailing[axis])) {
                        ...
                        node.layout[dim[axis]] - getPaddingAndBorderAxis(node, axis) - getMarginAxis(child, axis) - getPosition(child, leading[axis]) - getPosition(child, trailing[axis]),
                        // You never want to go smaller than padding
        ...
        while (endLine < node.children.length) {
            ...
            var /*float*/
            containerMainAxis = node.layout[dim[mainAxis]];
            ...
            for (i = startLine; i < endLine; ++i) {
                ...
                if (getPositionType(child) === CSS_POSITION_ABSOLUTE && isPosDefined(child, leading[crossAxis])) {
                    ...
                    var /*float*/
                    leadingCrossDim = getPaddingAndBorder(node, leading[crossAxis]);
                    ...
                    if (getPositionType(child) === CSS_POSITION_RELATIVE) {
                        ...
                        if (alignItem === CSS_ALIGN_STRETCH) {
                            ...
                            if (!isDimDefined(child, crossAxis)) {
                                child.layout[dim[crossAxis]] = fmaxf(
                                containerCrossAxis - getPaddingAndBorderAxis(node, crossAxis) - getMarginAxis(child, crossAxis),
                                // You never want to go smaller than padding
                                getPaddingAndBorderAxis(child, crossAxis));
                            }
